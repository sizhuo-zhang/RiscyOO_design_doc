...
\begin{itemize}
    ...
    \item Method \code{csrInstWr}: updates a CSR when a \code{csrrw} instruction is committed.
    ...
    \item Methods \code{sret} and \code{mret}: udpates the CSRs when a return-from-trap instruction is committed, and returns the next PC.
